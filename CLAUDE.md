...
A first, deliberately narrow slice of reporting/dashboards (FR-9) is also
in -- a doctor-only revenue summary, see "Reporting" below. Still not
built: the rest of FR-9 (a hospital-wide/admin view), and notifications/
digital delivery (FR-10).
...
- `reporting` — `getRevenueSummary` (BRS FR-9, doctor-only revenue summary, see "Reporting" below)
...
### Reporting (`src/app/doctor/reports`, `src/reporting`)

A first, deliberately narrow slice of BRS FR-9 -- a later, explicitly
requested addition, not the full hospital-wide dashboard FR-9 describes.
`getRevenueSummary` (`src/reporting/revenue-summary.ts`) is scoped to the
logged-in doctor's own visits/bills (via `Visit.doctorId`), the same way
every query on the doctor's home screen already is -- this lives on the
doctor's own screen (`/doctor/reports`, `requireSession(['DOCTOR'])`), not
a `HOSPITAL_ADMIN` one, per explicit request.

Shows two fixed periods -- **Today** and **This month** (IST calendar
boundaries, `getISTDayBoundsUTC`/the new `getISTMonthBoundsUTC` in
`src/shared/ist-date.ts`) -- no custom date-range picker for this first
pass. Each period reports patients seen, visit count, total revenue
collected, a **consultation fee vs. other charges vs. medicines**
breakdown, and a cash/UPI/card breakdown.

**Revenue collected (the headline figure, real money -- `Bill.totalCents`
summed over `PAID` bills) is deliberately not forced to equal the
consultation/other/medicines breakdown below it.** That breakdown sums
each `BillLineItem.lineTotalCents` -- the *pre*-discount/tax gross amount
billed -- so a bill with a discount or nonzero tax rate makes the two
numbers genuinely diverge; forcing them to match would mean proportionally
allocating discount/tax across line items, a materially more complex
calculation not attempted here. The page says so directly rather than
presenting numbers that quietly don't add up. Line items are classified by
matching `description` against `CONSULTATION_FEE_DESCRIPTION` (now
exported from `src/billing/consultation-fee.ts` specifically so this
can't drift out of sync with the literal string
`collectConsultationFee` actually writes) for the consultation-fee
bucket, `itemType === 'MEDICINE'` for the medicines bucket, and everything
else (other `SERVICE` line items -- front desk's surgery/procedure
charges, an optional service charge added at billing time) falls into
"other charges."

Verified directly against real seeded billing data, not just typechecked:
computed the expected Today/This-month totals by hand from `psql`
(including a case with an actual per-bill discount, confirming revenue
collected and the gross consultation-fee figure correctly diverge) and
confirmed the rendered page matched exactly.
